...
maz_ids = sorted( ( m_entry( * x ) for x in [
		...
		( 'xo_zetsu', 						10, 		[r'\bcomic +xo +zetsu\b'] ),

		( 'milf', 							0, 		[r'\bcomic +milf\b', 										r'コミック *ミルフ'] ),
...
ps = dict( ( k, list( map( reiu, v ) ) ) for k, v in  [
	( 'y', [ r'\b(?P<y>(?:19|20)\d{2})(?:-\d{2})', r'(?P<y>(?:19|20)\d{2})年\s*\d{1,2}月' ] ),
	( 'm', [ r'(?:19|20)\d{2}-(?P<m>\d{1,2})', r'年\s*(?P<m>\d{1,2})月号?' ] ),
...
def main():
	...
	for file in  itertools.chain.from_iterable( map( iglob, set( opts.filespec ) ) ):
		...
		if not any( info[x] for x in "ymv" ):
			continue
